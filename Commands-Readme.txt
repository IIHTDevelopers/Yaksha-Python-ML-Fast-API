* To run application for use case use following command(please install other python packages as you need)
	...
	a) cd into project folder<fastAPI>

	b) pip install -r requirements.txt --user

	c) uvicorn main:app --reload 
	    you can debug your APIs in Swagger UI.

	d) (optional)install any required libraries by – pip install pymodule --user

* Once you implement all methods, do

	pip-chill --no-version >> requirements.txt

* On command prompt, cd into your project folder (cd <fastAPI>), before running unit tests, do 
